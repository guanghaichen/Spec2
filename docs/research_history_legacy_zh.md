# SpecVLA-DFLASH 研究历程归档

> **历史资料，不是当前使用说明。** 本文保留从 Action-RNN、跨 Anchor 蒸馏、
> Domino 式课程、树验证、RAES 到当前方案的完整研究轨迹，其中包含已经否决、
> 被替代或仅用于消融的设计。当前正式方法、命令和结果以仓库根目录
> `README.md` 为唯一入口；请勿直接照本文中的旧命令启动新实验。
> 下方正文按当时认知原样归档，其中出现的“当前”“正式”“主线”均指对应历史阶段。
