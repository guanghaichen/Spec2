...
if [[ -n "${DFLASH_TEMPORAL_ROUTE_LABEL:-}" ]]; then
  HOLD_ROUTE_LABEL="${DFLASH_TEMPORAL_ROUTE_LABEL}"
else
  case "${DFLASH_TEMPORAL_HOLD_POLICY_OVERRIDE:-False}:${DFLASH_TEMPORAL_HOLD_POLICY:-adaptive}" in
    True:visual_budget) HOLD_ROUTE_LABEL="VisualBudget" ;;
    True:paced_budget) HOLD_ROUTE_LABEL="PacedBudget" ;;
    *) HOLD_ROUTE_LABEL="Adaptive" ;;
  esac
fi
export LOG_DIR="${USER_LOG_DIR:-${LOG_DIR}}/dflash_relaxed/简化版Draft+VTPF-TD-${HOLD_ROUTE_LABEL}-e${EVAL_EPOCH}"
...
# 保留通用入口的向后兼容默认值；最终 PacedHarmonic 启动器会显式提供
# H1/H2 共用 Target 锚点的逐深度视觉界。
export DFLASH_TEMPORAL_BYPASS_MAX_PIXEL_RELATIVE_L2="${DFLASH_TEMPORAL_BYPASS_MAX_PIXEL_RELATIVE_L2:-1.0}"
export DFLASH_TEMPORAL_BYPASS_USE_PIXEL_GUARD="${DFLASH_TEMPORAL_BYPASS_USE_PIXEL_GUARD:-False}"
export DFLASH_VERIFY_SKIP_MIN_STABLE_ACTIONS=1
export DFLASH_VERIFY_SKIP_MAX_CONSECUTIVE="${DFLASH_VERIFY_SKIP_MAX_CONSECUTIVE:-2}"
if [[ "${DFLASH_TEMPORAL_HOLD_POLICY_OVERRIDE:-False}" != "True" ]]; then
