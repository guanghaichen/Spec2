...
class MMModel(nn.Module):
    """
    Draft 模型（EAGLE/EAGLE2 风格的 EA layer）。

    设计目标：
    - 作为“大模型 base_model”的草稿生成器：基于当前上下文的 hidden/embedding，快速提出候选 token（topk + tree）。
    - 训练时不直接输出 token，而是输出 hidden_states；token 候选通过外部传入的 lm_head(head)投影得到。

    典型使用方式：
    - 训练：输入 teacher 的 last-layer hidden 序列 + “embedding/浅层表征”序列，监督 next-step last-layer hidden。
    - 推理：给定当前前缀，通过 topK_genrate 生成 draft_tokens（树形候选）以及 tree_mask/retrieve_indices 等结构，
      供 base model 并行 verify（speculative decoding）。

    维度依赖：
    - hidden_size / attention_heads / vocab_size 等来自 EConfig（train-scripts/llama_2_chat_7B_config.json）。
      该配置常用 num_hidden_layers=1，使 Draft 非常浅，推理成本显著低于 base model。
    """
    def __init__(self, config, load_emb=False, path=None, bias=True, total_tokens=50, depth=4, top_k=8, threshold=1.0):
        """
        Args:
            config: EConfig（LLaMA 风格配置），决定 hidden_size、num_hidden_layers 等。
            load_emb: 是否从 base model checkpoint 加载 embed_tokens 权重（Draft 与 base vocab 对齐）。
            path: base model checkpoint 路径（用于 load_emb 时查 index.json 并加载 embedding）。
            bias: fc 是否使用 bias。
            total_tokens: 生成树中最多保留多少个候选 token（会减 1，另一个位置留给 sample_token）。
            depth: 树展开深度（循环次数）。
            top_k: 每一层的分支数（topk）。
            threshold: 早停阈值（当前代码里只 log 了，实际 early-stop 注释掉了）。
        """
        super().__init__()
        ...
        # Draft 内部的 token embedding（默认随机初始化；训练/推理一般会 load_emb 对齐 base_model）
        ...
        self.top_k = top_k
        # total_tokens 实际用于“树节点数”（不含 sample_token），所以减 1
        self.total_tokens = total_tokens - 1
        self.depth = depth
        # 保留 log(threshold) 方便在 log-score 空间比较（当前 early-stop 分支已注释）
        self.threshold = math.log(threshold)
        # Draft 主体：极浅的 LLaMA Decoder（典型 config.num_hidden_layers=1）
        self.layers = nn.ModuleList([LlamaDecoderLayer(config, index) for index in range(config.num_hidden_layers)])
        # 将“(input_embeddings, hidden_states)”拼接后映射回 hidden_size，用于融合信息
    ...
    def init_tree(self):
        """
        初始化“树模式”需要的常量：
        - tree_mask_init: [1, 1, top_k, top_k] 的单位阵（bool），用于在扩展树时拼接。
        - position_ids:   [top_k]，用于给树中每一层设置 position_ids（相对位置）。
        """
    ...
    def reset(self):
        # 清空当前 tree_mask（注意力约束），让 Draft 回到普通自回归/非树模式
        self.tree_mask = None

    def _prepare_decoder_attention_mask(self, attention_mask, input_shape, inputs_embeds, past_key_values_length):
        """
        生成 Draft 的 attention mask：
        - 基础是 causal mask + padding mask；
        - 若设置了 self.tree_mask，则在“树验证区域”对注意力进行额外约束，实现 tree decoding。
        """
        # create causal mask
    ...
    def forward(
        ...
        """
        Draft 的前向：输出与 base_model 最后层对齐的 hidden_states 序列。

        Args:
            hidden_states:    [B, T, H] 输入的 teacher/baseline hidden（训练时来自 teacher last-layer hidden 序列）
            input_embeddings:[B, T, H] 输入的“embedding/浅层表征”（训练时来自 teacher 的第 0 层 hidden）
            attention_mask:  [B, T(+past)] padding mask（内部会扩展并与 causal/tree mask 结合）
            past_key_values: 用于 use_cache=True 时的 KV cache（Draft 在 topK_genrate 中会复用缓存）
            position_ids:    [B, T] 可显式指定 position ids（树展开时会用到）
            use_cache:       若 True，则返回 (hidden_states, next_decoder_cache)
            std:             可选噪声注入到 input_embeddings（数据增强/鲁棒性）

        Returns:
            - 若 use_cache=False：hidden_states [B, T, H]
            - 若 use_cache=True ：(hidden_states [B, T, H], next_decoder_cache)
        """
        ...
        # 这里不从 input_ids lookup embedding，而是直接用外部提供的 input_embeddings
        inputs_embeds = input_embeddings.to(hidden_states.dtype)
        # 融合“embedding + hidden”后进入浅层 decoder
    ...
    def reset_kv(self):
        # 清空 Draft 的“稳定 KV cache”。topK_genrate 会在多轮调用时复用/增量更新该缓存。
        self.stable_kv = None

    @torch.no_grad()
    def topK_genrate(self, hidden_states, input_ids,input_embeds, head, logits_processor):
        """
        生成树形 draft 候选 Token（核心接口，推理用）。

        输入（推理常见形状）：
        - hidden_states: [1, T, H] 当前前缀对应的 hidden（来自 teacher/base_model 或上一轮更新后的 hidden）
        - input_ids:     [1, T]    当前前缀 token ids（通常包含文本 prompt + 已接受 token）
        - input_embeds:  [1, T, H] 当前前缀的 token embedding（通常来自 base embedding 或 teacher 第 0 层 hidden）
        - head:          lm_head，将 hidden -> vocab logits，用于 topk 选 token

        输出：
        - draft_tokens:      [1, 1 + total_tokens] 树中选出的 token（首位是 sample_token；后续是候选节点 token）
        - retrieve_indices:  [leaf_num, max_depth] 把“叶子路径”映射回 draft_tokens 索引（用于并行验证重排）
        - tree_mask:         [1, 1, 1+total_tokens, 1+total_tokens] tree attention mask（bool/float）
        - tree_position_ids: [1, 1+total_tokens] 每个节点的树深度（用于 position_ids）

        备注：
        - 本函数会在内部进行 depth 次展开：每层对所有分支做一次 Draft forward，再 topk 组合成下一层分支。
        - stable_kv 用于缓存前缀，减少重复计算。
        """
        self.training=False
        ...
        depth = self.depth
        top_k = self.top_k
        # sample_token：当前前缀的最后一个 token（作为树的“根 token”，拼到 draft_tokens 的第 0 个位置）
        sample_token = input_ids[:, -1].to(hidden_states.device)
        # 训练/推理中经常会在开头放特殊 token，这里直接丢掉第 0 位，与后续的 input_ids[:,1:] 对齐
        ...
        if hasattr(self, "stable_kv") and self.stable_kv is not None:
            ...
            out_hidden, past_key_values = self(hidden_states, input_embeddings = input_embeds,use_cache=True)
        self.stable_kv = past_key_values
        last_hidden = out_hidden[:, -1]
        last_headout = head(last_hidden)

        last_p = self.logsoftmax(last_headout)
        # 第一层分支：在当前 last_hidden 上做 top_k，得到初始候选 token（每个分支一条 path）
        ...
        # input_hidden: [1, top_k, H]，表示“每个分支共享同一个 last_hidden”作为下一步输入
        ...
        for i in range(depth):
            ...
            # cu_scores: 把历史分支的 log-score（scores）与当前步 log-score（topk_p）相加，形成所有扩展候选的累积分数
            cu_scores = topk_p + scores[:, None]

            # 从 (top_k * top_k) 个扩展候选里再选 top_k 个作为下一层的“活跃分支”
            ...
            # tree_mask 逐层扩展：选中上一层的活跃分支 out_ids，并在末尾拼接一块单位阵（给新扩展出来的 top_k 节点）
            tree_mask = torch.cat((tree_mask[:, :, out_ids], self.tree_mask_init), dim=3)


        scores_list = torch.cat(scores_list, dim=0).view(-1)
        ss_token_list = torch.cat(ss_token, dim=0).view(-1)
        # 从整棵树中选出总计 total_tokens 个“分数最高”的节点作为最终 draft 节点集合（后续 verify 只验证这些节点）
        ...
        # retrieve_indices：为每个叶子构造一条从根到叶的路径索引（用于把 tree candidates “展开成候选序列”）
